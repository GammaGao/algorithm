 ...
 * NOTE:The array may contain duplicates.(可能有重复元素)
 *
 * TIME:2015-5-17
 * TYPE:binary search
 * FROM:leetcode
...
* 性能：时间复杂度O(logn)，注意可能有重复元素, 其性能退化到O(n)
*/
class Solution {
public:
	int findMin(vector<int>& nums) {
		...
		while (q - p != 1){
			int mid = (p + q) / 2;
			if (nums[mid] == nums[p] && nums[mid] == nums[q]) //重复元素
				return findLinear(nums);
			if (nums[mid] >= nums[p]) //取等号时nums[mid] == nums[p] != nums[q]
