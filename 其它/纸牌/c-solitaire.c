...
/* State of the 54-card deck.  This keeps a spare deck for copying
   ...
   deck: two so the deck can be moved backward if a joker(百搭牌) is moved
   ...
   somewhere even when there's a joker on the top of the pack(一幅纸牌). */
...
/* Cycle the state for "rounds" outputs, skipping jokers
   ...
   immediately before the large batch(大批) run. */
...
)
{
    ...
    do {
        assert(state.a != state.b); //if state.a == state.b, error and exit
...
/* Key(锁) the deck with a passphrase(密码). */
